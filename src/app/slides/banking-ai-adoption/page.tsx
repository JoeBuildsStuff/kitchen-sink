...
const percentFormatter: Formatter<number, string> = (value) =>
  typeof value === "number" ? `${value}%` : `${value ?? ""}`

const currencyFormatter: Formatter<number, string> = (value) =>
  typeof value === "number" ? `$${value.toFixed(1)}B` : `${value ?? ""}`
            ...
            <CardContent className="space-y-6">
              ...
              <ChartContainer config={adoptionConfig} className="w-full">
                <AreaChart data={adoptionData} margin={{ left: 12, right: 12, top: 12, bottom: 12 }}>
                  ...
                  <ChartTooltip
                    content={
                      <ChartTooltipContent formatter={percentFormatter} />
                    }
                  />
